...
const MessageInput = ({ chatId }: MessageInputProps) => {
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault(); // 기본 줄 바꿈 동장 박지
      ...
      <form className="message-form">
        ...
        <div className="input-container">
          ...
          <textarea
            ...
            onChange={handleTextareaChange} // 텍스트 변경시 높이 조절 및 저장
            onKeyDown={handleKeyPress} // Enter 처리 (Shift + Enter 예외처리)
            className="message-textarea"
            rows={1}
            placeholder="메시지를 입력하세요... (Enter로 전송, Shift+Enter 로 줄바꿈)"
            disabled={sending} // 전송중일때 비활성화
          />
          {/* 메시지 전송 버튼 */}
          <button
            type="submit"
            className="send-button"
            disabled={!message.trim() || sending} // 빈 메시지거나 전송 중일때 비활성화
          >
            {sending ? (
              <>
                {/* 전송 중일때 로딩 스피너 표시 */}
