...
- 이메일 가입 회원과 SNS 가입 회원이 email 이 겹치면
- 인증 환경 셋팅은 자주 변경됩니다.
...
- 생성된 앱 `할일 서비스` > 앱) 일반 메뉴 선택 > 개인 개발자 비즈 앱 > 개인 개발자 비즈 앱 전환
- 플랫폼 > 웹 플랫폼 등록 > 사이트 등록 > `http://localhost:5173`

### 1.2. 반드시 Rest API 키를 보관해둠.

- `388d0211cc622b06d0f117512221c4ec`
...
- 카카오 로그인 메뉴 > 일반 > `Client Secret 활성`
- 시크릿 키 보관 : `is5oOjQoEbtTSdSzqpC7nyhoKWno42br`

## 2. Supabase 인증 셋팅하기

- supabase.com > 프로젝트 > Authentication 메뉴 > Sign In / Providers 메뉴
- Auth Providers 항목에서 `kakao` 활성
- Rest Api 키 와 Client Secret 키 입력해줌
- Callback URL (for OAuth) 의 내용을 반드시 보관해 둠.
- `https://ukunsknavgnzobcmjqqp.supabase.co/auth/v1/callback`
...
function AuthCallback() {
  ...
  const handleAuthCallback = async (): Promise<void> => {
    try {
      // URL 에서 세션(웹브라우저 종료시 사라지는 데이터)에 담겨진 정보를 가져옮
      ...
      if (data.session?.user) {
        ...
        const { data: existingProfile } = await supabase
          ...
          .eq('id', user.id)
          .single();
        // 존재하지 안흔 id 이고, nickName 내용이 있다면
        ...
        setMsg('인증 정보 자체가 없습니다. 다시 가입해 주세요.');
      }
    } catch (err) {
...
- Supabase 대시보드 > API 메뉴 > Stored Procedures(프로시저) 목록 확인
  <img width="226" height="149" alt="Image" src="https://github.com/user-attachments/assets/e61ef7df-c984-4556-af88-65948eaea7e1" />
...
const uploadAvatar = async (file: File, userId: string): Promise<string | null> => {
  try {
    ...
    throw new Error(`아바타 업로드 오류가 발생했습니다. : ${err}`);
  }
};
...
- 카카오 로그인 시 인증코드 가 제대로 작동 안되는 경우 발생
