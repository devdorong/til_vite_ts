...
- 확장 타입
- 하나의 메시지는 메시지 타입과 발신자 정보가 혼합이 되어있음.
- 아래는 샘플링
...
```ts
// 성공 응답
const successResponsse:any<데이터형이 변경가능> = {
  success:true,
  data: 데이터형에 맞는 데이터
}
 ...
 *  각 목록에는 마지막 메시지, 읽지 않은 메시지 수, 상대방 정보 포함
 */

const mockChats: ChatListItem[] = [
  {
    id: 'chat1',
    name: '김철수', // 채팅방이름인데 1:1 이라서 상대방 닉네임으로
    type: 'direct',
    last_message: {
      // 마지막 메시지 정보(선택사항)
      content: '안녕하세요', // 내용
    ...
    updated_at: new Date(Date.now() - 1000 * 60 * 30).toISOString(), //  마지막 에ㅓㅂ데이트 시간
  },
  {
    id: 'chat2',
    name: '이영희', // 채팅방이름인데 1:1 이라서 상대방 닉네임으로
    type: 'direct',
    last_message: {
      // 마지막 메시지 정보(선택사항)
...
/**
 * Mock 메시지 데이터
 *  실제로는 Supabase 의 메시지 테이블에서 가져올 데이터 샘플
 *  채팅방 ID 를 키로 해서 각 채팅방의 메시지를 그룹화후 관리
 */
...
export async function sendMessage(
  ...
  // 시뮬레이션으로 백엔드 비동기 느낌으로 0.5초 진행 대기
  ...
  const newMessage: Message = {
    ...
    content: messageData.constent, // 메시지 내용
...
export async function getMessage(chatId: string): Promise<ChatApiResponse<MessageDetail[]>> {
  // 시뮬레이션으로 백엔드 비동기 느낌으로 0.5초 진행 대기
  await new Promise(resolve => setTimeout(resolve, 500));
 ...
 */
export async function searchUser(searchTerm: string): Promise<ChatApiResponse<ChatUser[]>> {
  ...
  return { success: true, data: filterdUser };
}
```

## 11. Chat 관련 Context API 로 상태 관리
...
import {
  ...
  searchUser as searchUsersService,
  findOrCreateDirectChat,
} from '../services/chat/directChatService';

/**
 * DirectChatContext의 Context 타입 정의
 * state 의 모양
 * action 의 모양
 */
...
// 컨텍스트 생성
const DirectChatContext = createContext<null | DirectChatContextType>(null);

// Provider 의 Props
interface DirectChatContextProviderProps {
  children?: React.ReactNode;
}
// Provider 생성
export const DirectChatContextProvider: React.FC<DirectChatContextProviderProps> = ({
  children,
}) => {
  // 상태관리
    ...
    async (participantId: string): Promise<string | null> => {
      try {
        ...
        if (response.success && response.data) {
          // 채팅방 새로 고침으로 목록 갱신
          await loadChats();
          return response.data.id; // 새 채팅ID 를 전달한 이유는 즉시 채팅방
        } else {
...
- /src/components/chat/DirectChatList.tsx 업데이트 및 활용
...
const DirectChatList = ({ onChatSelect, onCreateChat, selectedChatId }: DirectChatListProp) => {
  // Context 활용
  const {
    loadChats,
    createDirectChat,
  ...
  const [showUserSearch, setShowUserSearch] = useState(false); // 사용자 검색 UI 표시 여부

  // 최초에 컴포넌트 마운트시 채팅 목록 로드
  useEffect(() => {
    loadChats();
  }, [loadChats]); // 신규 또는 메시지 전송 등으로 업데이트 시 목록 호출
...
- /src/components/chat/DirectChatRoom.tsx 업데이트 및 활용
- Context 를 통해서 Service API 호출 및 상태 출력
...
const DirectChatRoom = ({ chatId }: DirectChatRoomProps) => {
  ...
  // 새 메시지가 추가될 때 마다 최신 메시지를 볼 수 있도록 해야 함.
  const messageEndRef = useRef<HTMLDivElement>(null);
  const scrollToBottom = () => {
    messageEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  };

  // 채팅방 ID 가 변경되면 메시지를 다시 로드
  useEffect(() => {
    if (chatId) {
      loadMessages(chatId);
    }
  ...
  // 현재 사용자 ID (지금은 Mock 버전이라서 current 라고 함)
