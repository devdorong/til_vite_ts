...
interface RichTextEditorProps {
  ...
  onImagesChnage?: (images: File[]) => void; // 파일을 보관하는 용도
}
const RichTextEditor = ({
  ...
  disabled = false,
  onImagesChnage, // 외부로 이미지를 전달하는 함수
}: RichTextEditorProps) => {
  // ref 변수들을 저장해둠.
  // ReactQuill 을 보관해둡니다.
  ...
  const imageHandler = useCallback(() => {
    ...
    // 여러개는 업데이트 예정
    // input.setAttribute('multifle', 'true');
    ...
    input.onchange = async () => {
      ...
      if (quill) {
        ...
        try {
          ...
          img.setAttribute('data-emp-id', tempId);
          ...
          try {
            const imgHtml = `<img src=${tempUrl} data-temp-id=${tempId} style={{maxWidth:"100%", height:"auto", margin: "10px 0"}} />`;
  ...
  useEffect(() => {
    if (onImagesChnage) {
      ...
      onImagesChnage(imagesFiles);
    }
  }, [onImagesChnage, value]); // 에디터에 내용이 바뀔때마다 이미지 목록 업데이트
...
- 삭제는 DB 삭제시 먼저 파일을 삭제하고, 내용을 삭제한다.
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <RichTextEditor
            ...
            placeholder="상세 내용을 입력하세요. (선택옵션)"
            disabled={saving}
            onImagesChnage={handleImageChange}
          />
        </div>
        {/* 추가정보 출력 */}
        <div
