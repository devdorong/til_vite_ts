  ...
  return (
    <div className="chat-page">
      ...
      <div className="chat-container">
        {/* 왼쪽 사이드바 - 채팅 목록 표시 */}
        <div className="chat-sidebar">사이드바 컴포넌트</div>
        {/* 오른쪽 메인 영역 - 채팅방 또는 화면 표시 */}
        <div className="chat-main">메인 컴포넌트</div>
      </div>
    </div>
  );
}
...
/* 채팅 상대방 아바타 컨테이너 읽지 않는 메시지 배치 위치 기준 */
.chat-avatar {
  position: relative;
  margin-right: 16px;
}
...
.chat-avatar .avatar-placeholder {
  ...
  height: 64px;
  border-radius: 50%;
  background: linear-gradient(135deg, #ee3900, #ff7e55);
  color: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
...
- `/src/components/chat/dirct 폴더` 생성
- `/src/components/chat/dirct/DirectChatList.tsx 파일` 생성
...
- `/src/components/chat/dirct/DirectChatRoom.tsx 파일` 생성
