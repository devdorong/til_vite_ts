...
export interface DirectChat {
  ...
  user1_id: string; // 참여자 1번의 id(UUID)
  user2_id: string; // 참여자 2번의 id(UUID)
...
export interface ChatState {
  currentChatId?: string; // 현재 활성화된 채팅방 ID
...
- `/src/contexts/DirectContext.tsx` 업데이트
...
const DirectChatRoom = ({ chatId }: DirectChatRoomProps) => {
  ...
  // 날짜 구분선도 표시
  // 사용자가 만약 채팅방을 한개 선택하면 각 채팅방의 메시지 내용이 들어옴
