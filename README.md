...
- id 는 URL 의 앞쪽 단어가 ID 가 됨.
  - id는 supabase URL 의 앞단어
  - 메뉴로 진입시 `Project Settings` > `General settings`
- package.json 수정

```json
"generate-types": "npx supabase gen types typescript --project-id 본인의id --schema public > 파일명.ts"
```
...
create table profiles (
  ...
  email text unique,
  role text check (role in ('admin', 'manger', 'member', 'guest')) default 'member',
  created_at timestamp default now()
);
