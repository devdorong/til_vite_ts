...
- 상세 내용 작성 부분을 Reqct Quill 을 활용함.
- 상세 내용은 til_npm 의 15-react-qull 참조
- 내용이 길어지므로 DB 에서는 칼럼이 `Text` 타입 권장
...
- src/pages/TodoInfinityPage.tsx
...
function AdminPage() {
  ...
  const approveDelete = async (id: string, updateUser: DeleteRequestUpdate): Promise<any> => {
    try {
      ...
      alert(`사용자 ${id}의 계정 삭제요청이 승인되었습니다. \n\n 관리자님 수동으로 삭제하세요.`);
      // 목록 다시 읽기
      loadDeleteMember();
    } catch (err) {
      console.log('탈퇴승인이 오류 :', err);
      return;
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <h2 className="page-title">👨‍✈️ 관리자 페이지</h2>
